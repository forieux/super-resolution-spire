function [Hrond Hdirect] = computeRI(lstep, bindex, lband, cl, ...
...
bound = round(5*sigma(1)*max(lband)./astep)*astep;
bound = 50; % Fix for no variation of the support in myopic estimation.
SupAlpha = -bound:astep:bound;

bound = round(5*sigma(1)*max(lband)./bstep)*bstep;
bound = 50;
