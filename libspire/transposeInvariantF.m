function object = transposeInvariantF(data, Hrond, index, Nalpha, ...
                                     Nbeta, Norder, Nbolo, Nspeed, ...
                                     Nscan, uspeed, theSpeeds, varargin)
  %% TRANSPOSEINVARIANT - transpose of the invariant spire model in
  %% Fourier space
  %%
  ...
  %% FUNCTION CALL
  %%
  %% [object] = transposeInvariant(Data, Params, Hrond250, Hrond360,
  ...
  %% respectively.
  %%
  %% Hrond250 (360/520) : a tab of Nalpha, Nbeta, Norder, Nspeed that
  ...
  %% respecting the zero convention of the fft2 (when you do Hrond =
  ...
  for iscan = 1:Nscan
    ...
    speed_index = find(uspeed(1,:) == theSpeeds(1,iscan));
