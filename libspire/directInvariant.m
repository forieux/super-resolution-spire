function data  = directInvariant(sky, Hrond, index, Nalpha, Nbeta, ...
  ...
  %% FUNCTION CALL
  %%
  %% data = directInvariant(sky, bands, Hrond250, Hrond360, Hrond520,
  ...
  for iorder = 0:Norder-1
    ...
    iorder_f = ufft2(sky(:,:,iorder+1));
    
    for ispeed = 1:Nspeed
      
      %% The convolution
      iorder_s = real(uifft2(iorder_f.*Hrond(:,:, iorder+1, ispeed)));
