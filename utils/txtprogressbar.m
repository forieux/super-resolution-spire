function txtprogressbar(fraction_done, message)
...
if (fraction_done == 0)
    progress_msg = ' 0%';
    progress_msg = sprintf('%s | %2.1f%% | %s remaining', message, 0, num2str(NaN));
else
    percent_done = 100*fraction_done;
    run_time = etime(clock, start_time);
    time_left = run_time/fraction_done - run_time;
    progress_msg = sprintf('%s | %2.1f%% | %s remaining', message, percent_done, sec2timestr(time_left));
end

% Display text
clc;
disp(progress_msg);
lmsg = length(progress_msg);
disp(char(cat(2, '#'*ones(1, floor(lmsg*percent_done/100)),'.'*ones(1, floor(lmsg*(100-percent_done)/100)))))
